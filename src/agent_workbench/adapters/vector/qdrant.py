...
class QdrantVectorIndex:
    ...
    async def search_hybrid(
        ...
        *,
        ...
        Deliberately not built from the public ``search``/``search_sparse``
        below, though they issue the same two queries. Those carry guards this
        method does not -- an empty principal set short-circuits, a limit below
        one is refused -- and adopting them here would be a behaviour change
        smuggled in under a refactor. The arms are shared at the level where
        the two really are identical: the query and the ordering.
        """
        ...
        dense_arm, sparse_arm = await asyncio.gather(
            self._dense_arm(vector=vector, narrowing=narrowing, limit=dense_limit),
            self._sparse_arm(
        ...
        return fused(dense_arm, sparse_arm, limit=limit)

    async def _dense_arm(
        self,
        *,
        vector: tuple[float, ...],
        narrowing: models.Filter,
        limit: int,
    ) -> tuple[ScoredChunk, ...]:
        """One raw dense arm: nothing has fused it, and its order is settled.

        The narrowing arrives already built rather than being assembled here,
        because a hybrid query must narrow both arms identically -- two
        constructions are two chances for the sparse arm to see a different
        candidate set than the dense one, and a difference there is a
        difference in who can be retrieved.
        """
    ...
    async def _sparse_arm(
        ...
        *,
        ...
        """One raw sparse arm. See ``_dense_arm`` for why the filter is passed in."""
    ...
    async def search_sparse(
        ...
        *,
        ...
        """The lexical arm on its own, unfused.

        Exists so a retriever that fuses more than two arms can obtain this
        one raw. ``search_hybrid`` returns a *fused* list, and fusing that
        again would produce an ordering no retriever produced -- the thing
        ``VectorIndexPort`` forbids in words. Guards mirror ``search``: the
        two are the same kind of method and differ only in which vector they
        query.
        """
...
def _ranked(points: Any) -> tuple[ScoredChunk, ...]:
    """One arm's response, mapped out of Qdrant's shapes and then ordered.

    Two jobs that used to be one function, and only the second moved: reading
    a payload is what this adapter knows and ``fusion.ranked`` must not, while
    the ordering is the part a graph-nominated arm has to share (ADR-033's
    reproducibility argument applies to every arm, not just these two).
    """

    return ranked(_scored(point) for point in points)
