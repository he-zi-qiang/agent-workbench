...
def _find_checkout_root(module_file: Path) -> Path | None:
    for candidate in module_file.resolve().parents:
        if (candidate / "pyproject.toml").is_file() and (
            candidate / "config" / "config.default.toml"
        ).is_file():
            return candidate
    return None
...
CONFIG_DIR = (
    CHECKOUT_ROOT / "config" if CHECKOUT_ROOT is not None else PACKAGE_ROOT / "_config"
)
DEFAULT_CONFIG_FILE = CONFIG_DIR / "config.default.toml"
TEST_CONFIG_FILE = CONFIG_DIR / "config.test.toml"
PRODUCTION_CONFIG_FILE = CONFIG_DIR / "config.production.toml"
