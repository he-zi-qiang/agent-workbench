...
- [x] **1.1 接一个模型 Provider**（2026-07-31 实测，DeepSeek `deepseek-chat`）
  ...
  - [~] **1.3 Agentic 检索**：**根因已消除，但没能复验**。模型确实自己发起了
        检索——事件里有 3 次 `ToolProposed`/`PermissionResolved`/`ToolStarted`——
        但 3 次全是 `ToolFailed: knowledge_search exceeded its 30s timeout`。
        原因见 4.1：进程里第一次 sparse 编码要 29.4s，工具超时是 30s。
        预热修好之后请求路径降到 0.32s，**但复验时 DeepSeek key 已失效**
        （公开贴出后被吊销），所以这一条仍未看到跑通。
        重新配一个 key 后应当直接可用，不需要改代码。
...
**`1.3` 复验（要新 key）→ `2.1` → 第 5 节。**

`4.1` 已查清并修掉了挡住 1.3 的那一半，所以顺序又变了一次——这是第三版，
两次改动都来自真跑而不是读代码。`1.1`、`1.2`、`1.4`、`1.5`、`3.1`、`4.1`
已在 2026-07-31 实测通过。

**这台 8 GB 机器上的操作提示**：不要同时跑多个持有 BGE-M3 的进程（API、摄取
worker、评测脚本各自加载一份）。要跑评测就先把另外两个停掉。
