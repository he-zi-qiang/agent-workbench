...
**2026-09-13 起镜像里没有 uv 的下载缓存了。** 那天在同样构建出来的 18.8 GB 镜像里量过，其中 5.2 GB 是它，不是
embedding 运行时；改了之后同样的构建，镜像 18.8 → 10.3 GB，浏览器镜像 20.5 → 11.8 GB。那份缓存挪进了构建缓存
（`Dockerfile` 里的缓存挂载），所以合计不能按镜像少掉的数直接往下减——合计没有重新量过，下面两条下限也就没动。
