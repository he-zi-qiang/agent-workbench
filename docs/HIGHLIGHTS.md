...
四行都实测于 `docs/closing-scan-2026-08-31` 这条**尚未合并**的分支上，**2026-08-31**
（ADR-097 接线之后）。同日稍早在 `main` 的 `a3619f9` 上测过一次，那组是**接线前**的：
`3981 / 3193 / 1376 / 826`。两组都记在这里，因为差额本身有意义——后端两行各 +8，
正是本批新增的 `test_candidate_funnel.py`；`main` 那组不含它。
...
| 环境 | 结果 |
|---|---|
| 后端，真实 PostgreSQL + Qdrant（本机） | `3989 passed / 12 skipped` |
| 后端，不起任何外部服务（本机） | `3201 passed / 800 skipped` |
...
**规模**：Python 源码 80498 行、测试 97829 行、前端 TypeScript 50555 行
