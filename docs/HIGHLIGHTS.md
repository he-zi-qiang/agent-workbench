...
最后一行实测于 **2026-09-01**（第六十七批那棵树），此前各行实测于 **2026-08-31**。
同日量过五组，差额本身有意义：
...
| 第六十七批（能力清单，本表） | **4088** | **3300** | **1398** | **857** |

**最后一行的 +56 / +56 / +22 / +15 有一半不是它自己的。** 上一次刷新这张表之后，
...
| 环境 | 结果 |
|---|---|
| 后端，真实 PostgreSQL + Qdrant（本机，空载） | `4088 passed / 12 skipped`（21 分 29 秒） |
| 后端，不起任何外部服务（本机） | `3300 passed / 800 skipped`（1 分 37 秒） |
| 后端，CI 那组服务型目录（`contracts`/`persistence`/`api`/`vector`/`e2e`） | `1398 passed / 2 skipped`（20 分 34 秒）※ |
| 前端 Vitest（本机 55 个文件） | `857 passed` |

这四行实测于 **2026-09-01**、第六十七批那棵树，机器空载。上一版（2026-08-31、
第六十六批）是 `4032 / 3244 / 1376 / 842`，两版之间还夹着 12 次没有登记数字的改动，
差额的归属见表下那一段。
...
`engines` 钉死的 node `24.14.0`。这次的 857 也是**本机**跑出来的，用的 node 与上一版同一个：
...
"CI 与本机跑的是同一条命令、同一组环境闸门"能拿出的最直接证据。**2026-08-31 与 2026-09-01 两版都只重测了
本机（1376、1398），没有对应的 CI 运行可比**，所以"逐位相同"是一句**上次成立**的话，
...
（只数 git 跟踪的文件）；`docs/adr/` 下 **89** 份，编号 0012–0102——**不连续**：0050 与
...
"87 份、0012–0100" 又被 ADR-101 与 ADR-102 甩在后面。这一行每加一份 ADR 就过期一次，
