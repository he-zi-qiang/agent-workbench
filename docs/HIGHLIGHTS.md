...
最后两行实测于 **2026-09-01**（各自那一批的树），此前各行实测于 **2026-08-31**。
同日量过五组，差额本身有意义：
...
| 第六十八批（零件开关，本表） | **4122**※※ | **3334** | **1408** | **863** |

**最后一行的 +34 / +34 / +10 / +6 全部是它自己的**，而且逐个文件对得上：
...
| 后端，不起任何外部服务（本机） | `3334 passed / 800 skipped`（1 分 19 秒） |
| 后端，CI 那组服务型目录（`contracts`/`persistence`/`api`/`vector`/`e2e`） | `1408 passed / 2 skipped`（13 分 19 秒）※ |
| 前端 Vitest（本机 55 个文件，空载重跑） | `863 passed` |

这四行实测于 **2026-09-01** 晚、第六十八批那棵树，机器空载；第一行同一棵树上还有一次
带负载的红跑（※※）。上一版（同日、第六十七批）是 `4088 / 3300 / 1398 / 857`，差额全部是本批
自己的测试，逐个文件的归属见表下那一段。
...
静态门禁全绿：`ruff format --check .`（635 files）、`ruff check .`、
...
**规模**（2026-09-01 第六十七批那棵树）：Python 源码 **82002** 行 / 324 文件、
测试 **100556** 行 / 265 文件、前端 TypeScript **51976** 行 / 142 文件
（只数 git 跟踪的文件）；`docs/adr/` 下 **90** 份，编号 0012–0103——**不连续**：0050 与
