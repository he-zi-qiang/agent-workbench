...
| 第五十八～六十三批（本次收尾，本表） | **4013** | **3225** | **1376** | **842** |
...
| 环境 | 结果 |
|---|---|
| 后端，真实 PostgreSQL + Qdrant（本机，空载） | `4013 passed / 12 skipped` |
| 后端，不起任何外部服务（本机） | `3225 passed / 800 skipped` |
...
（只数 git 跟踪的文件）；`docs/adr/` 下 **85** 份，编号 0012–0098——**不连续**：0050 与
...
"84 份、0012–0097"又被 ADR-098 甩在后面——
