...
| 第六十四批（边界守卫改白名单，本表） | **4017** | **3229** | **1376** | **842** |
...
最后一行的 +4 是 ADR-099 那四条白名单守门测试。**五目录那一行仍然重测了**，
...
| 环境 | 结果 |
|---|---|
| 后端，真实 PostgreSQL + Qdrant（本机，空载） | `4017 passed / 12 skipped` |
| 后端，不起任何外部服务（本机） | `3229 passed / 800 skipped` |
...
**规模**（2026-08-31 重量）：Python 源码 **80685** 行 / 320 文件、
测试 **98456** 行 / 256 文件、前端 TypeScript **51064** 行 / 139 文件
（只数 git 跟踪的文件）；`docs/adr/` 下 **86** 份，编号 0012–0099——**不连续**：0050 与
...
> **上一版这三个规模数是 80542 / 97925 / 50613，而同一节的表格与它们同批写下、
> 却只刷新了一半**：那一批里 `828` 与 `800` 是新的，`97925` 是旧的。
> 「同一提交里一半刷新一半没刷新」正是本节反复处理的那件事，记在这里而不是抹掉。
