...
| [ADR-106 权重只住在一个进程里，其余进程去问它](./0106-one-process-holds-the-weights-and-the-others-ask-it.md) | ADR-0105 让四个进程各加载一整套检索模型，Windows 启动器因此要 29 GB 才肯构建，而一台 32 GB 的机器默认只给 Docker 16 GB；dense 与 sparse 还是同一份权重装了两次 | 接受：新进程 `agent-encoder` 加载三个模型一次并在五条路由上服务；三个远程适配器各实现一个已有 port，连接时做本地适配器加载时的每项检查外加一项身份比对；两片默认空的叶子 `rag.embedding.service_url` / `rag.reranker.service_url`（不抬 schema）；Compose 只让 `encoder` 挂权重卷，另外四个等它 healthy；内存线 29 / 51 改 12 / 16。**推翻 ADR-0105 §3.4 的前提。** 原生路径这次没接上 |
| [ADR-107 沙箱 broker 独占 Docker socket，经回环隧道被访问](./0107-the-sandbox-broker-alone-holds-the-socket.md) | ADR-0105 §4.1 说沙箱要 socket、挂进 API/Worker 会抵消 `cap_drop: ALL`、「那笔交易需要它自己的 ADR」——这就是那份 ADR | 接受：一个只跑 `agent-sandbox-mcp`、不挂 key、不挂产物、不读配置的容器持有 socket（root + 硬化锚点原样）；主镜像从 `docker:29-cli` 复制一个静态 CLI；`loopback_proxy.py` 参数化后在 API/Worker 容器里各起一条 `127.0.0.1:8766 → sandbox:8766` 的隧道，三道回环守卫一字不改地成立；`docker/decide_sandbox.py` 让两个启动器探 broker 运行时后才导出开关，profile 里留 `false`；`stack.cmd` 加 `sandbox-image`、`restart` 多重启 broker。**推翻 ADR-0105 §4.1 第一条** |
| [ADR-108 Windows 的屏幕适配器自己合成那张图](./0108-a-screen-adapter-for-windows-composes-its-own-frame.md) | ADR-0105 §4.1 说 computer use「补不上」——对容器拓扑永远成立，但屏幕适配器从 ADR-070 起就是「一个平台一个薄模块」，缺的只是第二个模块和一个把它放到桌面所在机器上跑的办法 | 接受：`adapters/screen/win32.py` 用 ctypes 直连 user32/gdi32/dwmapi/shcore、零新绑定（Windows 这一半 extra 只有 Pillow）；`exclude_native` 靠「逐扇 `PrintWindow` 已批准窗口再合成」兑现而不是靠过滤，代价是被遮挡的已批准窗口完整可见；per-monitor-v2 DPI 让点即物理像素；激活不敲 Alt（ADR-091 §2.3）；`SendInput` 的返回值被数，UIPI 拒绝是错误不是成功；同意用 `MessageBoxTimeoutW` 默认「否」；server 在主机上由 `scripts\computer.cmd` 起，API 经 `host.docker.internal` 隧道读 `/session`。**碰屏幕的那一半一次也没在 Windows 上跑过**；新登记 F-35、F-36 |
| [ADR-109 容器这条路上还有两样装配得出的：版面预览，和一个能写的文件夹](./0109-a-container-lays-the-page-out-and-hands-a-session-one-folder.md) | 一位 Windows 用户报「没有浏览器和 shell，word 预览、代码执行都做不到」：`stack.cmd` 照 Dockerfile 默认构建所以版面预览恒 503；文件夹选择器打开在镜像的只读树 `/app` 里，选得中写不进；原生档的 `project_run` 是宿主 shell，Compose 档没有也不该有，而控制台没有一行说 Code 够得到什么 | 接受：启动器传 `--build-arg WITH_FIDELITY_PREVIEW=1`（`lite` 不传，Dockerfile 默认不变）；只有 `api` 把宿主 `var/projects` 绑在 `/projects`，新叶子 `code.projects_root` 让选择器从那里打开；能力清单新增 `code.sandbox` / `code.host_commands` / `code.web_search` / `artifact.layout_preview` 四行；Code 起始屏在第一句指令前画「这里能碰到」一行。**拒绝把 `project_run` 放进容器**（ADR-0077 的前提与 ADR-0057 的「不提供兑现不了的工具」），登记 F-37 |
