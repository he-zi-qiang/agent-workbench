...
加了 `activate_application`，于是这一道从「这扇窗是**人**放到前面的吗」变成
「这扇窗是**人批准过的那一组里的**一扇吗」。重读、不缓存、每个动作之前——这三件事一个
字没动，上面这段照旧成立；弱掉的是「人的当下选择」那一层额外信息。让这个交换成立的
收窄写在那份 ADR 的 §2.2：**当前前台应用不在 allowlist 里时，拒绝激活**。
...
adapters/screen/darwin.py  Quartz / CGEvent，全仓唯一碰 pyobjc 的文件
```
...
`darwin.py` 顶部有逐条列出的 pyright 抑制而不是切成 basic 模式——pyobjc 没有类型
信息，严格模式在这个文件里报 186 个 unknown，一条都不是这个项目能处理的缺陷；
但严格模式**还能说的**（未定义的名字、不可达分支、对本模块自己函数的错误调用）
仍然要红。这正是把 FFI 压进一个薄模块的意义。

> **2026-08-29 更正，两处，而且这两处不是「当时为真、后来变假」——它们在写下的那一天
> 就是假的。**
>
> 一、上面这段原文写的是「**六条**逐条列出的抑制」。本 ADR 落地的那次提交
> （`bf2ffc7`）里，`darwin.py` 抑制的是 **7 条** `report*` 规则（4 行注释，逗号分隔）。
> 数字从一开始就少一条。ADR-092 之后是 8 条。
>
> 二、原文还写着「**全仓库只有这一个文件带抑制**」，上面那个分层块里也写着「全仓唯一
> 一个 pyright 抑制的文件」。**这条从来没有成立过**：
> `apps/word_mcp/renderer.py` 自 `7bdde20`（MCP Adapter 落地那批）起就带着 4 行文件级
> 抑制，而 `7bdde20` **早于** `bf2ffc7`。也就是说，写下这句话的时候，仓库里已经有
> 第二个这样的文件了。
>
> 真正成立、也是这段真正想说的那件事，比原话窄：**全仓只有这一个文件碰 pyobjc**，
> 所以平台 FFI 的爆炸半径是一个薄模块。ADR-092 把 AppKit 的 run loop 放进这里而不是
> 放进需要它的入口点，依据的是收窄之后的这句，不是原来那句。
>
> 按 `known-gaps.md` 的四分类，这属于**口径不实**——一句读起来像保证的话，读者拿它
> 判断不了代码，和 F-26 同型。发现它的是另一个会话的复核，落地记录在
> [status.md](../status.md) 第四十八批。
...
忘掉的检查。六个工具全部走同一个 `ScreenGate`，四道检查只有一份。（ADR-091 之后是
八个工具，这条理由原样适用，激活那两道检查也在 gate 里。）
...
**这一段把原理说对了，而它描述的那道检查直到 2026-08-29 才存在。** 写本 ADR 那一批
落地的 `DarwinScreen.__init__` 只 preflight 了**录屏**；辅助功能被这段散文点名、被
`config.computer-local.toml` 写进「任何一个缺了就在启动时退出」，实现里一次也没查。
2026-08-29 在真机上撞上了它：录屏已授权、辅助功能未授权，于是服务器正常启动，
`activateWithOptions_` 返回 `True` 而前台一动不动，十次尝试、十秒预算都一样——点击和
按键会是同一个结果，而工具照常回一句成功。补上 `CGPreflightPostEventAccess` 那道检查
之后，这段话才是对这个仓库的描述而不是对一个想法的描述。落地记录在
[status.md](../status.md) 第四十六批。
