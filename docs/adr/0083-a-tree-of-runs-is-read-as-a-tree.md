...
> **后续（2026-08-27）**：「参与的 Agent」面板现在可以折叠子树了，而这**不是**本节推迟
> 的那件事——两处折叠的代价不同。本节要避免的是改 `StepStream`：那是 Work、Chat、Code
> 每个阶段共用的组件，一次改动波及三个页面。面板里的折叠只动面板自己，所以它拿得到
> 本节说的好处而不付本节说的代价。**步骤流里的可折叠分组仍然没有做。**
