...
```text
全局导航（rail，272px，八个页面同一个宽度，不随内容变）
├── 工作空间：对话 / 任务 / 编码   ← 三个 primary，永远是链接，当前那一项只多一个 aria-current
├── 资源：知识库 · 待处理          ← 待处理是等你批准的任务，从任何一页都看得见（只列 Task 的审批）
└── 最近记录区                     ← 当前模式的列表另起一区，带自己的标题行和折叠箭头

（2026-09-05 起。上一版把列表嵌在它的导航项底下、导航项兼作折叠开关，于是 Chat 激活时
Tasks 和 Code 被推到栏底、辅助页整栏收窄——2026-09-04 评审第 1 条；见 docs/reviews/。）
...
├── 任务（Tasks）              ← primary
│   ├── 标题下的行动区：按状态给停止 / 去批准 / 按原输入重新提交 / 复制诊断；落定后结果在前、过程折成一行
│   ├── Task 列表与提交
│   ├── TaskInput artifact
│   ├── 节点时间线与未知事件
...
├── 编码（Code）               ← primary
│   ├── 起始屏：选一个文件夹，或就地新建一个（ADR-074；选择器浏览的是服务端的目录）
│   ├── 服务端会话列表，名字来自第一句指令（ADR-047），每行常显时间
│   ├── 工作区文件：点开看正文，或下载
│   └── 本轮步骤（持久事件，延迟下限是一个轮询周期）：
│       思考在它促成的动作上面；动作行展开是参数、返回、没成是为什么；
│       没跑完的一轮在报告的位置说为什么；整轮的事件记录一条一行
├── 知识库
│   ├── declare → raw PUT → complete
│   └── /v1/search 检索检查
├── 用量
├── 效果评测
├── 计算机（首屏：状态、在这台部署上怎么起服务器、重新检查；规则折在「工程说明」里）
├── 运行状态（演示前自检、健康检查、Worker 登记两格、能力清单与开关，身份三行只读）
└── 设置（对话框，左下角头像；只装能改的：模型密钥（默认）、外观、本地身份＝开发与权限演示）
```

「更多」是页面的入口（知识库在窄屏、用量、效果评测、计算机、运行状态、快速跳转），
外加一行「设置」。设置对话框里**不**再装用量和运行状态：它们是页面，装进来就是同一份
报表画两遍，而且是在一个为表单定尺寸的框里画一张六列的表。主题只在设置的「外观」里。
