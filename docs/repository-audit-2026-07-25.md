...
> 上列四个阻断面已全部修复并有回归测试，逐条记录见 §7（P0-1、P0-2、P1-1、
> P1-3、P1-2、P1-5）。剩余 P1-4、P1-6～P1-10 不属于这四条。
...
### P1-5 tool/token/cost budget 不是硬上限 —— 已修复（2026-07-26）
...
### P1-8 重复 tool_call_id 在副作用之后才被拒绝 —— 已修复（2026-07-26）
...
### P1-5 Runtime 预算硬上限（2026-07-26）

三处独立的行为变化。共同的形状是：**上限必须在花费之前生效**——事后记账的不是
上限，副作用已经发生了。

**一、tool-call 配额在 dispatch 之前预留。** 此前一轮提出 N 个调用就全部执行，
之后才把 `len(turn.calls)` 记进账；余量只剩 1 时提出 3 个，3 个 handler 全跑，
账上还写 3——超过了上限本身。现在按 `max_tool_calls - usage.tool_calls` 计算准入
数，超出的调用立即以 `budget_exceeded` 拒绝（仍然发 `ToolProposed`，因为模型确实
提议了，也仍然回答它的 id，因为每个 id 都欠一个结果）。**只对准入的调用记账**：
因上限被拒的调用不该自己消耗上限，否则账面会报出超过预算的花费。

**二、每回合合并 usage 之后复检预算。** 循环顶部那次检查发生在回合之前，看不到
这一回合花了什么。于是 `max_total_tokens=1` 而模型上报 120 tokens 时，run 报的是
`completed`；若这一回合还提议了工具，工具照跑。复检放在 `_terminal_for_turn` 里
错误/取消分支之后、`if turn.calls` 之前，因此两种后果同时消掉：不会假装完成，
也不会在超限之后再跑工具。

顺带的行为变化：步数用尽的那一回合，其工具调用不再执行。最后一步给了模型，工具
结果已经没有读者，执行它们等于用真实副作用换取会被丢弃的输出——和 tool 上限是同
一个缺陷换个位置。已用测试钉住。

**三、无法度量的成本上限被拒绝。** `cost_micro_usd` 没有任何生产者，成本预算永远
停在 0、永远不触发。这与 P0-2 的 `requires_approval` 是同一种形状：一个只写字段，
沉默的方向恰好是放行。这里不实现计价器（那需要按 model revision 的价格表，属于
独立变更），而是**拒绝接受一个无法执行的上限**——调用方要的是一个保证，静默地不
提供它比说出来更糟。引入计价器的那个 PR 负责删掉这个分支，不是在那之前。

**明确的残留**：token 上限仍然只能在模型调用**之后**核对——一次调用要花多少
token，调用之前不可知。所以单次调用可以冲过上限，之后 run 立刻失败。上限约束的是
run 走多远，不是单次 provider 调用返回多少；后者是 provider 侧的 `max_tokens`，
`turn.finish == "max_tokens"` 已另行处理。计价器仍未实现，成本预算目前只能拒绝，
不能执行。

回归测试 10 条（4 tool 上限 + 3 token 上限 + 1 步数上限 + 2 成本上限，含 3 条
对照）。**验证过是有牙的**：撤掉配额预留失败 3 条；撤掉回合后复检失败 3 条；
撤掉成本拒绝失败 1 条；把记账改回 `len(turn.calls)` 失败 1 条——四个方面各自独立
覆盖。
...
### P1-8 重复 tool_call_id 在派发前被拒绝（2026-07-26）

行为变化两处。

**一、模型 turn 完成后、准备/授权/执行之前检查 ID 唯一性。** `tool_call_id`
是「结果回答的是哪一次调用」这件事本身；两次调用共用一个，就没有办法说哪个结果
属于哪一次。此前只有 `align_results()` 会发现，而它跑在 handler **之后**——于是
模型重复一个 ID，工具就按重复次数各执行一次，run 再死在记账上。复现确认：
**两次 handler 调用**。现在整轮失败，handler 零调用。

唯一性是**每轮**的，不是全 run 的：ID 只需要在同侪之间可区分。跨轮复用同一个 ID
照常执行，有测试钉住。

**二、`align_results()` 的失败归一化为终态 outcome。** 复现确认了审计的补充说明：
`ToolPairingError` **逃出了 `run()`**，调用方拿到 traceback 而不是终态
`AgentOutcome`。这违反 `AgentExecutor` 「必须返回终态 outcome 而不是对可预期失败
抛异常」的协议约定，也让 Graph node 拿不到可记录、可路由的结果——与 P1-7 里
policy 异常逃逸是同一种形状。`_run_tool_batch` 现在返回 `AgentOutcome | None`。

回归测试 5 条（含 2 条对照：一轮内不同 ID 照常并行执行、跨轮复用同一 ID 不算重复）。

关于那条 backstop：预检使重复 ID 永远到不了 `align_results()`，所以第二处修改在
正常路径上**不可达**。不可达的分支如果没人跑过，它是一个主张而不是保证——因此
测试里放了一个故意坏掉的 gateway（返回带错误 `tool_call_id` 的结果）来实际走一遍。
生产代码里没有任何东西会这么做。
