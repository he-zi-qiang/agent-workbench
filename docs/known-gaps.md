...
| C-08 | `high` 档下委派任务撞 token 天花板：子代理撞 `max_output_tokens`，父运行撞 `max_tokens_per_agent_invocation` | **失败中** |
...
### C-08 `high` 档下的委派任务跑不完：撞的是 token 天花板
...
**为什么不在第三十五批顺手改掉**：那批的依据是一次量出来的调用时长分布，而这三个数要动
哪一个、动到多少，需要的是**另一份测量**——一个 `high` 档 analyst 的回答长度分布，以及
一个父运行在含三到四次委派时的累计用量分布。今天只有 n=1。照着一次运行去调三个预算，
正是这个仓库在 `max_output_tokens` 那条注释里拒绝过的做法（"别往上填没量过的"）。

**做完的判据**：拿到那两份分布，然后动其中**一个**并说明为什么是它——尤其要先回答
"16384 是不是本来就该更大"，还是"一个 analyst 该被要求写得更短"。后者是提示词的问题，
不是预算的问题，而这两条通向完全不同的改动。
