...
> **待办（尚未登记为独立条目）**：评审契约要求模型原样抄回 32 位十六进制的
> `reviewed_draft_ref`。节点自己已经知道这个值，让模型复述它没有任何上行收益，却带着
> ②那种一字之差的下行风险。可选的修法（都要 ADR）：不要求回抄、由节点自己绑定；或
> 解码时对该字段做等价性之外的处理。**在有 ADR 之前不动它**——它是一条授权相关的绑定，
> 「评的是不是这一份草稿」正是这个字段存在的理由。
