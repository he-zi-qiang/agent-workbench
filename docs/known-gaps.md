...
> 那条删除真正卡在 [B-12](#b-12-检查点里的图状态没有任何迁移路径--未实现) 上。
...
| `TaskStep.depends_on` | 计划里的步骤依赖，被四道校验守着、`_PLAN_CONTRACT` 明确要求模型产出 | **口径已改诚实，删除移交 [B-12](#b-12-检查点里的图状态没有任何迁移路径--未实现)。** 字段现在写明「记录并校验，从不执行」——计划是给模型的建议，不是任何东西调度的 DAG。删不掉的原因是真的，且**没有任何机制会修它**：`TaskState.plan` 从检查点重建且 `extra="forbid"`，而事件 upcaster 走的是另一条路 |
...
**第七处 `depends_on` 移交 [B-12](#b-12-检查点里的图状态没有任何迁移路径--未实现)**，
并且它已经不在「看起来像被守着」的状态了：字段自己写着「记录并校验，从不执行」。
移交而不是继续开着，是因为**挡住它的不是这条缺口**——删字段会让部署时在飞的
Task 全部无法恢复，而**没有任何机制会修它**。
...
### B-11 核心层的第三方依赖守卫是黑名单 —— **已关闭**（[ADR-099](./adr/0099-a-denylist-cannot-say-no-to-what-nobody-listed.md)，2026-08-31）
...
1. 检查点侧有自己的迁移路径——在 `_to_state` 里按状态自带的 `schema_version` 升级，
   与事件那条**分开**（两条路存的东西不一样，共用一个注册表会得到一个键对不上的表），
   并有一条对着真实旧检查点行的测试；
2. 或者写明**域模型变更前必须排空在飞的 Task**，并让某个东西执行这条排空
   ——一句没人执行的规矩，和今天没有规矩是同一件事。

**在那之前，任何对 `TaskState` 内部模型的字段改动都要按"会中断在飞任务"来对待**，
包括 B-10 移交过来的那次删除。
