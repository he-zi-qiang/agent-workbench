...
| F-14 | Task 工作集里的文件只在事件里有名字，界面上完全不可见 | 已知代价 |
| F-15 | 运行产出卡片对 listing 时序有可见的不一致 | 已知代价 |
...
### F-14 Task 工作集里的文件在界面上完全不可见 —— 已知代价
...
**眼下**：ADR-066 里排过这条的次优解——把名字**列出来并明说打不开**——诚实且不需要
任何新端点，但本批没有做。写在这里是因为「完全沉默」和「列出来说打不开」之间的差别
不需要一份 ADR，只需要一次实现。

**做完的判据**：Work 页能说出一个 Task 往工作集写了哪些文件；能不能打开它们是第二
个问题，可以继续答「不能」。
...
**为什么不修**：修它的正路是让运行的响应自带结构化条目（name/media_type/size_bytes）。
那要改 `RunFileResponse`，而 `SandboxOutcome.written` 是工具与路由共用的那一半
（ADR-065 §3 明说必须共用），工具侧直接把它喂给 `ToolResult.workspace_writes`
（`tuple[str, ...]`）。一次要前后端同步部署的响应形状变更，换一件退化路径已经诚实
覆盖了的事，代价倒挂。

**做完的判据**：运行返回之后的第一次渲染就能画出卡片，或者这条不一致被读者实际
抱怨过、因而值得那次形状变更。
