...
| F-43 | 人操作模型那个浏览器走的是 250 ms 一张的截图轮询，仲裁按进程而不是按会话：能点按钮、能按方向键看结果，打不了动作游戏，任何会话有回合在跑就点不了 | 已知代价 |
| F-44 | Code 会话跨回合只带用户消息与最终报告，工具结果不带：每一轮都要重读项目（一轮 338k 输入 token），失败的一轮连报告都不留，下一轮不知道上一轮为什么失败 | 缺口 |
| F-45 | 步数或调用数用尽的一轮 `RunFailed` 且不留报告：文件写好了、bug 修了，控制台只有「max_steps」，下一轮从零来 | 缺口 |
...
### F-43 人操作模型的浏览器走的是截图轮询，仲裁按进程 —— 已知代价
...
**做完的判据**：真实转录里出现「想在面板里连按却卡顿」的反馈时再评估推流；仲裁细到会话要先有
每会话一个浏览器上下文（ADR-0113 §4 第二条同时翻案）。
...
**该怎么做**：ADR-0116 给「工具被收走」的一轮留了一次不带工具的收尾调用，让模型写报告；步数用尽
是同一种处境——预算花完了，但模型知道自己做到哪了。把那一次收尾调用也给它（不计入步数，或者
把 `max_steps` 的最后一步永远留给收尾），`RunCompleted` 带 `stop_reason=max_steps` 而不是失败。
一条自己的 ADR，因为它改的是运行时对预算耗尽的语义。

**做完的判据**：一轮在 `max_steps` 上结束时，会话里有它的报告，控制台的那一行说的是「做到哪、
没做完什么」，而不是只有一个上限的名字。
