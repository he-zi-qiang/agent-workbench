...
| C-08 | 委派任务撞 token 天花板——**父运行把委派叙述了一遍**，同一份工作做了两次 | **失败中** |
...
### C-08 委派任务跑不完：父运行把委派叙述了一遍，同一份工作做了两次
...
**第一次改动已落地，但还没被验证到**：`delegate_agent` 的工具描述现在明写「委派就是这次
调用本身，不要描述委派、不要预告要用哪几个子代理、更不要自己把子代理的结论写出来」。
重跑那条 objective 时它**没跑到 `work` 就死了**（见下），所以这句话是否真的管用，目前
没有证据。别把它记成已验证。

**第二张面孔，实测下来是同一种病**：重跑死在 `understand` 上，`the model stopped at its
output token ceiling`——`max_output_tokens = 16384`。量了一下这个库：

| | |
|---|---:|
| `high` 落地（2026-08-26）以来，撞输出上限的运行失败 | **7 / 14** |
| 单次运行累计输出 token p50 | 1,506 |
| 同 p90 | 13,742 |
| 同 max | 29,247 |

一半的运行失败是撞在这一个数上。但**不要因此就去调大它**，理由有两条：

1. `config.demo-local.toml` 自己的注释写着，`max_output_tokens` 除了给长回答留空间，
   **也是跑飞那一轮唯一的刹车**。调大它就是把刹车放松。
2. 更要紧的是：`understand` 这一步该产出的是一份简短的理解，而它写了超过 16,384 个
   输出 token。这跟父运行把三份分析扮演着写一遍**是同一种病**——模型在该给结论的地方
   写长文。放宽上限买到的是"让它写得更长"，不是"让它写得对"。

**做完的判据**（修订）：先让这两处的提示词把"这一步要产出什么、多长"说清楚，再重跑同一条
objective 拿到一次真的跑完的委派任务。只有在那之后，"16384 是不是本来就该更大"才是一个
能被单独回答的问题。
...
**证据**：[darwin.py](../src/agent_workbench/adapters/screen/darwin.py) 的 `capture`
返回的 `Capture.width/height` 是**那一块显示器**的尺寸（`displays()` 逐个报自己的点
尺寸），而同一个适配器的 `click`／`move`／`scroll` 用
`Quartz.CGEventCreateMouseEvent` 发的是**全局坐标空间**的事件。

单显示器上两者重合，所以一直没出事。**接第二块显示器就会错位**：模型照着
`screenshot(display_id=N)` 那张图量出 `(x, y)`，发出去的点击落在全局坐标的
`(x, y)`——也就是主显示器上的某个位置。没有任何东西会报错，点击会**成功**，只是落在
别处。

这条是对照 Claude Desktop 时发现的：它的 `computer_batch` 明写「本批坐标一律指向调用
前那张全屏图」，还配了 `switch_display` 让截图跟着显示器走，正是因为这个歧义真实存在。
我们把**单位**钉死了（点、左上原点、从不说像素，见
[ports/screen.py](../src/agent_workbench/ports/screen.py)），**参照系没钉**。

[ADR-076](./adr/0076-a-window-nobody-approved-is-not-in-the-picture.md) §4 记了这条并
明确不在那一轮修——那批已经在动同意模型和抓屏机制两处。

**做完的判据**：一个坐标要么带着它所属的那次 capture 的身份，要么统一成全局空间；两块
