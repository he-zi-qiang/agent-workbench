...
| C-06 | 面板只认页面握着的事件：深链、刷新保持、"这棵树完不完整"三件都缺 | 未实现 |
| C-07 | 收窄到一个子运行时，阶段列表仍报全任务的进度 | 未实现 |
...
**为什么不是顺手能做的**：第一项要把一个查询参数接进 `WorkPage` 的路由，而深链进一个
**不在当前页**的子运行必须走 `/runs`——那正是文件头说不从这里发的请求。两条不矛盾（实时
用事件、冷启动用端点），但"什么时候用哪个"是一条需要写下来的规则，不是一次顺手的接线。
...
**做完的判据**：被收窄掉的阶段自己说得出"这一段里没有这个运行的步骤"，而不是显示成
一个做完了却什么都没做的阶段。要改的是 `StepStream` 这个**每个阶段共用**的组件，所以
它是一次会波及 Chat 与 Code 的改动，不是 Work 页里的局部调整。
