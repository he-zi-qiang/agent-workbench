...
| B-07 | ~~tool 参数读不出来时，说不出是被截断还是真的坏~~ | **已关闭**（2026-09-12） |
...
| B-12 | 检查点里的图状态没有任何迁移路径 | ~~未实现~~ **已关闭**（ADR-100，2026-08-31） |
| B-13 | 五目录跑法下 `test_code_api.py` 间歇性红 5 条，**未归因** | 失败中 |
...
> **2026-08-31 当天写下、当天撤回的一条补记。** 那条补记说 `TaskStep.depends_on`
> 的删除移交到这里，是「升级链第一个具体的对象」。**它是错的，而且错在把两条路
> 认成了一条**：这个注册表由 `PostgresEventLog` 应用在**存储的事件信封**上、发生在
> 回放的时候（`event_log.py:514`），**而检查点从不经过它**。
> 那条删除真正卡在 [B-12](#b-12-检查点里的图状态没有任何迁移路径--已关闭adr-1002026-08-31) 上。
> 撤回而不是删掉这段，因为「把两条恢复路径认成一条」正是这份文档要防的那类错误，
> 而它这次发生在写这份文档的人身上。
...
### B-07 tool 参数读不出来时，运行时说不出是被截断还是真的坏 —— **已关闭**（2026-09-12）
...
**关闭记录（2026-09-12）**：判据逐条兑现。`_completed_tool_calls` 现在收 `finish` 与
provider 自报的那个字符串，`finish == "max_tokens"` 时报的是「模型在输出上限上把话说了
一半」并说出下一步（分几次要），其余情况一字未改仍报「provider 送来坏参数」；对照组在
`tests/contracts/test_deepseek_model.py`，两条用同一段坏 JSON、只差一个 finish_reason。

**那一位放在 message 里，不是一个新字段，这是个选择。** `ErrorInfo` 只有 code / message /
retryable，没有 details；为一个只有一处分支会读的字符串给 `ModelStreamCompleted` 加字段，
要连着动 port 与领域事件。message 会原样进事件日志，也会原样出现在控制台那句
「这一轮没有跑完（…）」里（`stopNote` 的兜底分支），所以从转录上判定「是被截断还是真的坏」
现在只需要读那一句。**也没有给它一个新的 `ErrorCode`**：`domain/errors.py` 把这个位置的
标准写得很明白——一个自己的词由「重试、换更小的提示、改代码都够不着」的情形挣得，而更小的
要求恰好够得着这一个。

**是什么把它从间歇变成常见的**：第七十七批给基础提示词加的那一段，让模型在「要一个能看的
东西」时写一个自包含的 `.html`——而一个文件的整份正文是走在**一次**工具调用的参数里的，
`config.code-local.toml` 的 32768 上限同时管思考与回答，那份 profile 自己的注释量过「最简单
的节点烧掉约 14,900 思考 token」。于是页面装不下、JSON 断在半句、控制台说 provider 送来了
垃圾。同一批因此在那段提示词里补上了它欠的那句话：大文件分几次写，骨架先落，段落用 edit
填——每次调用是一个新请求，各有各的上限。

**没做的**：这一轮仍然是死的，没有续写、没有重试。B-06 记着那件事，而它的前提（重复文本
谁来吞）这里一个字也没动。

**2026-09-13 补记（ADR-0118）**：「这一轮仍然是死的」不再成立。截断那一半现在不是错误：适配器交出
一个 `cut_off` 的空调用，运行时用一条拒绝作答给**模型**，同一轮继续；第三次才结束运行。上面那句
「分几次写，骨架先落，段落用 edit 填」也换掉了——模型复述过它、还是整个文件一次发，因为「文件末尾」
不是 `project_edit` 能命名的锚点；现在 `project_write` 有 `append`。坏 JSON 那一半照旧是
`provider_error`。
...
| ~~`TaskStep.depends_on`~~ | 计划里的步骤依赖，被四道校验守着、`_PLAN_CONTRACT` 明确要求模型产出 | **已删除**（[ADR-100](./adr/0100-a-checkpoint-versions-its-own-layout.md)），连同四道校验、`TaskPlan` 的第五条规则与 planner 契约里那一句。挡着它的 [B-12](#b-12-检查点里的图状态没有任何迁移路径--已关闭adr-1002026-08-31) 同批关闭：检查点从此有自己的升级链，而**这次删除就是那条链的第一个真实用户**——真实库里 85 个带计划的检查点，不迁移 0 个能载入 |
...
**七处全部满足**（2026-08-31）：两处接上（`WORKSPACE_WRITE_SCOPE`、id 词表）、
五处删掉（`TERMINAL_RUN_STATES`、两个前缀、`output_schema`、`ProjectPathSegment`、
`summarise_children`、`depends_on`），每一处都在代码里留下了**为什么**
——删掉一个符号而不留下它曾经声称过什么，等于把「这里错过」也一起删掉。

**第七处 `depends_on` 当天先移交、后关闭**，而中间那一步值得留着：它先被移交给
[B-12](#b-12-检查点里的图状态没有任何迁移路径--已关闭adr-1002026-08-31)
（检查点没有迁移路径），因为**挡住它的不是这条缺口**；B-12 随即由 ADR-100 关闭，
而这次删除就是那条新升级链的**第一个真实用户**——一个注册表为空的迁移机制，
正是它要防的那种东西。
一条缺口的判据里挂着另一条缺口的前置，就该由后者持有；后者做完了，前者才跟着关。

> **本条 2026-08-31 当天改过一次移交对象，记在这里。** 最初写的是移交 B-05
> （事件 upcaster 注册表为空）。那是**把两条恢复路径认成了一条**：upcaster 应用在
> 存储的事件信封上、发生在回放时，检查点从不经过它。查实之后新登记 B-12。

**2026-08-31 处置了两处**（上表已划掉），**判据是"这一处是冗余还是欠款"**：
前三个是冗余——删掉或接上即可，代价为零；后四个是**已经付过设计费而没有收到货**，
每一处都要先回答一个真问题（依赖关系该怎么排序执行、段名约束该守住哪个字段、
工具输出该按什么校验），所以它们留着。
`domain/identifiers.py` 那四个铸造器同属前一类，但它们还带着一处**前缀不一致**
（domain 声明 `"thread"`，实际产出 `"thr"`）——删掉之前要先确认没有任何持久化数据
依赖那个声明，那是一次要读迁移的检查，不是顺手做掉的事。

### B-11 核心层的第三方依赖守卫是黑名单 —— 已关闭（ADR-099，2026-08-31）

**已关闭**：[ADR-099](./adr/0099-a-denylist-cannot-say-no-to-what-nobody-listed.md)。

**留在正文而不是删掉**：它的论证是"白名单为什么值得"的全部依据，
而下一个想往核心层加依赖的人正需要读它。

**分类**：口径不实（2026-08-31 全仓扫描新登记，当天关闭）。
...
**做完的判据**（当时写的）：`domain/` 与 `ports/` 的第三方 import 改成**白名单**
（当前应为 `pydantic` 与 `regex` 两项，各附一行理由），
并让白名单之外的任何第三方 import 让测试失败。README 两版同步改口径。

**已满足**：`CORE_THIRD_PARTY_ALLOWLIST` 覆盖**整个核心层**（不止 `domain/` 与
`ports/`——它们今天的第三方依赖是同一组，拆成两张表只会得到两张一样的表加一次将来
会漂的同步，见 ADR-099 §4.3）。标准库判定走 `sys.stdlib_module_names` 而不是手写清单
——手写清单和这条白名单要消灭的错误是同一种形状。四条测试：

1. 核心层只许 import 标准库、自己与清单里的包；`regex` 还被钉死在
   `domain/workspace.py` 一个文件上。
2. **清单里的包若没人 import，测试失败**——一份只增不减的白名单就是换了顶帽子的黑名单。
3. 每条许可必须写明理由（`reason` 是必填字段，不是注释）。
4. 白名单与 `FORBIDDEN_CORE_IMPORTS` **不许相交**——两个守卫一条规则，
   否则将来某次编辑可以让"哪个测试先跑"决定构建是绿是红。

**两条对照组都实测过，而它们是这条关闭的实质**：
`regex` 出现在 `domain/workspace.py` 以外 → 白名单红、**黑名单绿**；
`attrs`（两张表都没见过）→ 白名单红、**黑名单绿**。
**这两次黑名单都说不出话**，那正是本条要修的东西。

**一行生产代码都没动**：核心层当前的第三方 import 只有 `pydantic` 与 `regex`。

### B-12 检查点里的图状态没有任何迁移路径 —— 已关闭（ADR-100，2026-08-31）

**已关闭**：[ADR-100](./adr/0100-a-checkpoint-versions-its-own-layout.md)。

**分类**：未实现（2026-08-31 收尾时查出，起因是一次**我自己写错的移交**：
[B-10](#b-10-核心层与契约里的八处死符号--已关闭2026-08-31) 的最后一处本来被移交给
B-05，而 B-05 管的是另一条路）。
...
| 有没有升级机制 | **有**：`EventUpcasterRegistry`，`event_log.py:514` 应用 | **没有**。`adapters/langgraph/` 里 `upcast` / `migrat` 零命中 |
| 注册表空不空 | 空（[B-05](#b-05-生产-upcaster-注册表为空--另一半-2026-08-12-已关闭)） | 不适用——机制本身不存在 |

**后果，实测而不是推断**（喂 `_to_state` 一份带未知键的 plan dict）：

```
REFUSED on resume: ValidationError
  1 validation error for TaskState
  ['Extra inputs are not permitted [type=extra_forbidden]']
```

也就是说：**改动 `TaskState` 里任何一个模型的任何一个字段——加、删、改名——
都会让部署时每一个在飞的 Task 无法恢复。** 不只是删。

**而 `schema_version` 让情况更糟，不是更好。** 它是 `VersionedModel` 上为迁移而存在的
字段，但 `reject_unsupported_schema_version` **失败关闭**：抬版之后，带旧版本号的检查点
被直接拒绝。同一次实测：
...
**爆炸半径最大的是 `waiting_approval`**：一个停在人工闸门前的 Task 可以在那儿待很久，
横跨若干次部署——而它正是最需要恢复得了的那种。今天没有人踩到，只是因为
`DOMAIN_SCHEMA_VERSION` 从来没动过（一直是 `1`），也就是说**这条路从未被走过一次**。

**做完的判据**（当时写的，二选一）：1. 检查点侧有自己的迁移路径，与事件那条分开，
并有一条对着真实旧检查点行的测试；2. 或者写明域模型变更前必须排空在飞的 Task，
并让某个东西执行这条排空。

**走的是第 1 条**（[ADR-100](./adr/0100-a-checkpoint-versions-its-own-layout.md)）：
`adapters/langgraph/checkpoint_migrations.py`，在 `_to_state` 这个**唯一漏斗**里
升级一次。**版本轴是检查点自己的，不是 `DOMAIN_SCHEMA_VERSION`**——判据里写的
「按状态自带的 `schema_version`」当时是错的，抬那个全局版本会让每一条已存事件
声称一个读者拒绝的版本，而能救它们的注册表是空的，**等于把整段事件历史隔离掉**。

**真实数据上的证据，而这正是这类机制一向缺的那一半**：本机开发库 218 个 `plan`
通道 blob，其中 **85 个非空、共 305 个计划步，每一个都带着 `depends_on`**。
...
也就是说：不做迁移而直接删字段，这台机器上**每一个带计划的检查点都会变成一个
恢复不了的 Task**。

**顺带关掉 [B-10](#b-10-核心层与契约里的八处死符号--已关闭2026-08-31) 的第七处**：
`TaskStep.depends_on` 已删除，连同它的四道校验、`TaskPlan` 的第五条规则，
以及 planner 契约里要求模型产出它的那一句。

**不替 B-05 结账**：拿到证据的是**检查点**这条路；事件那条的注册表仍然是空的、
仍然没在真实数据上走过。两条路，两份证据。

### B-13 五目录跑法下 `test_code_api.py` 间歇性红 5 条 —— 失败中，**未归因**

**分类**：失败中。不是"未实现"也不是"口径不实"——测试写对了，而**我没能查清它为什么红**。
登记的是这个状态本身，连同已经排除的东西，好让下一个人从数据开始而不是从零开始。

**症状**：`pytest tests/contracts tests/persistence tests/api tests/vector tests/e2e`
偶尔以 5 条失败结束，全部在 [tests/api/test_code_api.py](../tests/api/test_code_api.py)，
其中三条已知名字：`test_the_shipped_default_does_not_mount_them`、
`test_a_process_that_cannot_serve_code_says_so`、
`test_a_process_that_was_not_asked_for_code_stays_quiet`。

**同一条命令、同一棵树的实测**（2026-08-31 夜至 09-01）：

| 跑法 | 结果 | 用时 |
|---|---|---:|
| 五目录 | **5 failed** | 2:27 |
| 五目录 | **5 failed** | 2:27 |
| 五目录 | 1376 passed | 11:29 |
| 五目录 | 1376 passed | 25:10 |
| 五目录（模拟 embedding extra 缺席） | 1376 passed | 6:00 |
| 全套 + 真实服务（五目录的超集） | 4032 passed | 12:46 |
| `tests/api` 单跑 | 355 passed | 9:27 |
| `test_code_api.py` 单跑 | 51 passed | 10:43 |
| `contracts`+`persistence`+`test_code_api.py` ×3 | 930 passed ×3 | 9:55 / 15:23 / 19:06 |

**最刺眼的一条线索没解释掉**：失败那两次用时 **2:27**，比任何一次通过都快，
**也比模拟 embedding extra 缺席的 6:00 还快**。也就是说失败时有大量工作没有发生，
而不是某一步算错了。

**已经排除的**：

- **不是 CPU 负载。** 第二次失败发生在空机器上。（我第一次判断是负载，错了。）
- **不是确定性的顺序问题。** 同命令随后连过三次。（我第二次判断是隔离问题，也错了。）
- **不是随机顺序插件。** 没装 `pytest-randomly`，`addopts` 里没有乱序。
- **不是 embedding extra 缺席。** 模拟缺席后 1376 全过（而它确实解释了
  `test_code_api.py` 从 10:43 变成 2.35s——那些用例每个都装配整个 API）。
- **不是新增模块的 import 期副作用。** `checkpoint_migrations` 在 import 时
  `register(1, ...)`；实测重复 import 得到的是**新的模块对象与新的注册表**，不会抛。

**没拿到的**：**那 5 条的实际报错文本**。两次失败都只留了 tail，此后四次尝试
（含一次专门为捕获而设的循环）都没能复现。

**2026-09-01 补一条 CI 的证据，它把嫌疑收窄了一大截。** 推送后 CI 的
`Migrations, stores and end-to-end recovery` job 跑的正是这五个目录：
**1371 passed / 7 skipped，绿，用时 2 分 33 秒**。

那个用时是关键：**2:33 和本机失败那两次的 2:27 几乎一样**，而本机每一次**通过**都要
6 到 25 分钟。差别在于 `test_code_api.py` 的 `_booted` 每个用例都装配整个 API 并加载
BGE-M3——**CI 上根本没有 `embedding` extra（门禁按分层约定不装它），所以那一步从来
不发生**，于是 CI 天然走的是"没有嵌入运行时"那条快路径，并且通过。

于是最贴合全部观测的假设是：**那两次失败是嵌入运行时"没能装载"而不是"不存在"**
——一次干净的 `ImportError`（我模拟的那种）走的是有人写过的降级路径，
而一次装到一半的失败（MPS 显存不足、权重缓存被占）走的可能是没人写过的那条。
这解释了为什么它快、为什么间歇（那晚这台机器刚跑完 2.5 小时消融加多轮全套）、
为什么 CI 从不复现、也为什么后来不再出现。

**仍然是假设，不是结论**——我没有那 5 条的报错文本，也没能人为造出"装到一半失败"。
但它给了下一个人一个具体可试的东西，而这比"间歇性红"强。

**同时它把「是不是 ADR-100 的回归」这条压得很低**：CI 在同一棵树上跑同一组目录，绿。

**与 ADR-100 的时间关系，写下来但不当结论**：这两次失败都发生在检查点迁移那次改动
**之后**；而在它之前，同一条命令今晚已连过四次（第六十三～六十五批的门禁测量）。
**这不足以归因**：改动碰的是检查点读取与 `TaskStep`，而红的是 Code 路由——两者没有
共同代码路径；改动之后同一条命令又连过三次，全套（五目录的超集）前后各绿两次。
把它写成"回归"和写成"既有 flake"一样，都是我今晚已经犯过两次的那种错——**在拿到
报错之前先下结论**。

**做完的判据**（按先后）：

1. **拿到那 5 条的报错文本。** 在此之前一切归因都是猜。
2. 有了文本之后再判断是回归还是既有。**CI 已经把"回归"压得很低**（同一棵树、
   同一组目录、绿），所以下一步该试的是上面那条假设：在嵌入运行时**装到一半失败**
   的条件下跑 `test_code_api.py`——例如人为限制显存，或在装载中途中断它。
3. 无论哪种，这条测试的**运行代价**本身要处理：`_booted` 每次装配整个 API 并加载
   BGE-M3，所以这一个文件要 10 分钟。一个跑一次十分钟的用例，天然会掩盖它自己的
   间歇性——每复现一次要等十分钟，而这正是本条至今没有报错文本的直接原因。
...
### D-07 在控制台里存一把 key 不产生任何事件

**分类**：未实现。不是口径不实——没有任何一处文档声称它有审计轨迹；这条登记的是
[ADR-101](./adr/0101-the-console-may-hand-over-a-key-it-can-never-read-back.md) §6
自己点名的那个缺口。

**症状**：`PUT /v1/settings/provider-key` 写下一把 key，事件流里什么也没有。
事后没有任何办法回答「这把 key 是什么时候、由谁存的」——而它是这套系统里第一个、
目前也是唯一一个**被写进来的凭据**。

**2026-09-01 扩大范围**：[ADR-103](./adr/0103-an-optional-part-can-be-switched-from-the-console-for-the-next-start.md)
之后 `PUT /v1/system/switches/{id}` 也写配置——四个开关型零件的下次启动值——同样
不产生事件。它们不是凭据，但它们改变下一台部署的形状，而「谁拨的」这个问题的答案
和「谁存的 key」一样：在 ADR-044 的前提下本来就答不出来。同一条缺口，同一个前提，
不另开编号。

**为什么现在没做**：这台部署的事件流按 run 与 task 组织
（`stream_id` 是会话或任务，序号是 per-stream 无缺口的），而一次存 key 不属于任何一次
run。给它发事件需要先回答「它挂在哪条流上」，而那是一个比这条缺口本身更大的决定：
一条「部署自己的」事件流是可以成立的，但它会是第一条不由 run 或 task 拥有的流，
per-stream 序号、订阅与保留策略都要重新回答一遍。

**为什么仍然接受**：它的前提和 ADR-101 全篇的前提是同一条——
[ADR-044](./adr/0044-no-remote-no-production-identity.md)：只绑 loopback、
Identity Adapter 只信请求头。在那个前提下「谁存的」这个问题**本来就答不出来**：
请求头是请求者自己写的。所以先补审计轨迹，得到的是一条可以随便署名的记录，
那比没有记录更糟。
...
> **关闭的方式**：每个 Worker 进程自己往 `worker_presence` 表登记（谁、哪种、哪个 profile
> 装配、能力快照、心跳与过期），`GET /v1/system/workers` 读它，过期由读侧按数据库时钟判。
> 运行状态页把「任务与文档 Worker：状态未知」拆成两格，在线的说合成与否、失联的说最近一次
> 心跳在几秒前；页顶多一块「演示前自检」。**没做的**：Qdrant 仍然看不见（它没有登记，也不在
> `/health/ready` 里）；跨机器看不见（登记写进这台部署自己的库）；能力清单里 `task.worker`
> 那一行仍是 `unknown`，因为它报的是**这个进程**装配了什么，Worker 的事实在另一条路由上。
> 下面原文照留，作为这一条为什么曾经接受的记录。

**分类**：未实现。不是口径不实——[ADR-102](./adr/0102-a-deployment-says-what-it-could-not-assemble.md)
落地时，能力清单里 `task.worker` 这一行答的就是 `unknown`，并写明「从这里看不出来」。
这条登记的是那个 `unknown` 背后的东西。

**症状**：API 进程无法回答三个问题里的任何一个——有没有 Task Worker 在跑、它注册了
哪些图、它是不是 `--demo` 起的合成 Worker。三种状态在控制台上完全一样：任务提交成功、
列表里出现、然后要么被跑掉、要么一直 `queued`。

这不是假想。Docker 默认栈的两个 Worker 都带 `--demo`，于是控制台上的任务会跑完、会
`succeeded`，而它们一次真实模型调用都没有发生过、一件 MCP 工具都没有被发现过。一个
人要发现这件事，今天的路径是去读 `RunStarted` 事件的 `tool_names` 字段——一条要先知道
去哪看才看得见的证据。

**为什么现在没做**：它需要一条新的事实源。Worker 侧现在往 PostgreSQL 写的只有租约、
心跳与事件，而这三样都挂在**具体某个任务**上：一个还没认领过任何任务的 Worker 在库里
不留任何痕迹，一个刚死掉的 Worker 也不会主动消失（只有租约过期看得出来）。要答
「此刻有几个 Worker、各自能做什么」得有一张 Worker 注册表，带自己的心跳、过期与
能力快照——那是一个新的表、新的所有权、新的一致性问题（一个 Worker 报告的能力和它
实际注册的工具目录必须是同一份，否则这张表会比没有它更误导人）。

**为什么仍然接受**：`unknown` 这个答案本身是诚实的，而且带着能立刻用的补救
（`docker compose ps` / `docker compose logs task-worker`）。在没有注册表之前，一句
写明「这个进程看不见那个进程」的话，比一个猜出来的绿点强。
...
| F-41 | 同一条命令连发两次、中间什么也没跑，第二次按记录作答而不真跑——用同一条命令轮询的回合会拿到上一次的答案 | 已知代价 |
| F-42 | 「放手做」仍然拦下来的那几种命令形状是一张静态正则清单，绕过它的破坏性命令会直接跑（能赔掉的只是 runner 容器里的那个文件夹） | 已知代价 |
| F-43 | 人操作模型那个浏览器走的是截图轮询（焦点下 100 ms 一帧）：能按住方向键走、能跳、能拖，打不了要求反应的动作游戏。人和模型可以同时动同一页（ADR-0119），冲突靠告知而不是靠锁 | 已知代价 |
| F-44 | Code 会话跨回合只带用户消息与最终报告，工具结果不带：每一轮都要重读项目（一轮 338k 输入 token），失败的一轮连报告都不留，下一轮不知道上一轮为什么失败 | 缺口 |
| F-45 | ~~步数用尽的一轮不留报告~~ | **已关闭**（2026-09-13，ADR-0119） |
| F-46 | 只拦得住「只改数字」的循环：形状交替（A、B、A、B）或靠变字母一格一格量的，仍然只有每 25 次一句提醒 | 已知代价 |
...
**2026-09-13 补记（第九十一批）**：`config.compose-local.toml` 也降到 1 秒——Compose 栈是用户实际在用的那一台，
此前继承 10，控制台上的步骤列表十秒一跳、一次多出四行。同一种十二次调用的回合，持久事件的送达延迟中位
5.23 s → 0.48 s、最大 9.99 s → 0.98 s（[status.md](./status.md) 第九十一批）。下面「留着的口子」仍成立：
出厂默认还是 10，真正的修法仍要先写 ADR。
...
（[FilePreview.tsx](../web/src/features/code/FilePreview.tsx) 的 `fileKey`）。同名文件被
改写之后，这份缓存里仍然是上一版的字节，而没有任何东西按「这一轮写过哪些名字」去失效它。
读者点开一个他这一轮之前预览过的文件，看到的是旧正文，且屏幕上没有一处说它是旧的。

**这一条是 ADR-0112 顺手缩小、而没有消灭的**。自动预览会把它要打开的那**一个**页面的三份
缓存先失效再打开（[CodePage.tsx](../web/src/features/code/CodePage.tsx)），因为不这样做，
「改一下这个按钮」那种轮次弹出来的就是一张看起来最新的旧页面——比不弹更糟。其余文件没有被
一起失效，是一个明写的选择而不是疏漏：它们没有在屏幕上，而顺手修一半会让下一个读者以为
整件事被修好了。

**为什么不是一行就能修**：正确的修法是让一轮结束时的那次 `reload` 知道**这一轮写过哪些
名字**，而那份名单今天只存在于事件流的派生结果里（`turnBlocks.ts` 的 `produced`），`reload`
拿不到它；把它传进去等于让「重新读一次会话」依赖「把事件流解释成回合」，而后者是有意做成纯
函数的。另一条更干净的路是让清单里的条目带上一个内容标识（版本或 mtime），把它编进缓存键
——`WorkspaceEntryView` 今天只有 `name` / `size_bytes` / `media_type`，那是一次服务端改动。
...
`project_run`（F-37，拒绝）、没有沙箱（F-24，拒绝）、没有浏览器（F-39，未接线），于是用
`project_grep` 的正则量词做二分：**一轮 74 次搜索、它委派的 explorer 114 次**，撞步数上限，
量出来的还是错的（源码行带着缩进和引号）。`runtime/agent_runtime.py` 的同问熔断一次没响——
没有两次是同一个问题。

**这一条是三条「拒绝／未接线」的交集，不是新的缺口来源**：F-24、F-37、F-39 各自都有理由，
而三者同时成立时，这个回合就是一个**没有任何仪器**的编码 agent。Claude Code 从来不需要
面对这件事——它永远有 Bash。

**已落地的缓解（ADR-0114）**：提示词说清「搜索只找到、不度量」，委派默认不用，explorer
自己也被告知，运行时每把工具第 25 次调用附一句提醒。这些把「循环到步数上限」换成「读一次、
推一次、报告是读出来的」，**但没有给它任何能算的东西**。

**为什么不顺手打开容器内 `project_run`**：命令会跑在 API 容器里，`AW_*` 之外的环境原样继承
（`bootstrap/child_environment.py`），而那个容器挂着 key 卷——一条 `cat` 就能把 provider key
读进转录。这是能力变更，要自己的 ADR 与隔离论证。

**做完的判据**：F-24、F-37、F-39 任一关闭，或者一条新 ADR 给 Compose 档的编码回合一个
能跑一行代码的地方（有隔离论证的）。之后本条随之关闭；ADR-0114 的那几段提示词仍然成立
——有 shell 的回合也不该拿搜索当尺子，只是它不会想这么做。

### F-41 同一条命令连发两次，第二次按记录作答而不真跑 —— 已知代价

**证据**：[agent_runtime.py](../src/agent_workbench/runtime/agent_runtime.py) `_RunLedger.answered`
与 `world_version`（ADR-0116 §2.1）。一个签名（工具名 + 排序后的参数）被提出时，如果这一轮已经
答过它、而且从那次作答之后没有派发过任何风险不是 `read` 的调用，运行时不派发，把上一次的结果换上
新的 `tool_call_id` 交回去。`project_run` 自己是 `destructive`，会让版本号加一——但记录是在这一批
加过版本**之后**写的，所以一个命令自己的效果不算作它自己的重复：`sleep 3; curl localhost:8080`
连发两次，第二次拿到的是第一次的输出，端口没有被再问一次。

**为什么算已知代价而不是缺口**：这正是 ADR-0116 要的性质——2026-09-12 那一轮把同一条
`python3 -c` 命令连发了四次、每次一张审批卡，第二次起按记录作答是那十张卡变四张的全部机制。
轮询是它的反例：一条命令的答案会随时间变而命令本身不变。编码回合 360 秒的预算里这种形状少见，
而且绕法是零成本的——换一个 `sleep` 的秒数就是新问题。

**做完的判据**：要么在提示词里说一句「轮询时每次换个写法」（ADR-0114 的提示词是它的自然位置），
要么给 `ToolSpec` 加一个「答案随时间变」的声明让这种工具豁免——后者是 schema 变更，只在有第二个
需要它的工具时值得。两者都不急：先看真实转录里有没有人被它咬到。

### F-42 「放手做」的例外清单是静态正则，绕过它的破坏性命令会直接跑 —— 已知代价

**证据**：[commands.py](../src/agent_workbench/domain/commands.py) `STILL_ASKS`，六种形状
（ADR-0116 §2.3）：删掉整个目录、git 丢弃未提交的工作、改写共享历史、把网上下来的东西灌进 shell、
伸出项目目录之外、无限 fork。`tests/domain/test_commands.py` 钉了 78 条用例，包括命令位置的判断
（`grep -r sudo .` 不拦，`sh -c "rm -rf ."` 拦）。一条不长这六种样子的破坏性命令——
`find . -type f -delete`、`python3 -c "import shutil; shutil.rmtree('.')"`——在放手做的回合里不问就跑。

**为什么算已知代价**：Claude Code 的 auto 模式在这个位置放的是一个模型，这个仓库没有第二个模型可花，
也不该让第一个给自己的命令打分。清单短、能读、能测，而它的覆盖面是明说的。它能赔掉的东西由这一档
**在哪提供**限定：只在命令跑进 runner 容器的部署上（ADR-0115 §2：只挂项目文件夹、没有 key、
没有数据库地址、没有工件卷），所以最坏的情形是那个文件夹——而那个文件夹在放手做之前就已经是
模型可以逐个删空的（`project_delete` 在这一档下也不问）。原生路径上这一档不存在，ADR-0077 的
那句话原样成立。

**做完的判据**：清单按真实转录里被咬到的形状增补，每一条带用例；不追求穷尽。如果有一天要求
「放手做」也管住文件夹本身，答案不是更长的清单，是快照或版本控制（`git stash` 之类，进回合之前做）
——那是另一条 ADR。

### F-43 人操作模型的浏览器走的是截图轮询 —— 已知代价

**证据**：[BrowserFrame.tsx](../web/src/features/code/BrowserFrame.tsx) 有焦点时 250 ms 取一帧，
[browser_input.py](../src/agent_workbench/apps/api/routes/browser_input.py) 在
`code.turns_in_flight > 0` 时答 409（ADR-0117 §2.1）。一次点击的往返是「POST 输入 → 下一次取帧」，
最坏约 250 ms 加浏览器那一侧的渲染；按住方向键连发靠的是键盘重复事件逐个送，每个一次请求。

**为什么算已知代价而不是缺口**：这块面回答的问题是「模型停在哪一页、按一下会怎样」，不是「在这里
打一局」。要打一局，文件形态的页面有沙箱预览（读者自己的浏览器，零延迟），网址形态的页面读者自己
开浏览器。推流（WebSocket 上的 screencast，或 WebRTC）是另一条 ADR，而且要先量真实的需求。
按进程仲裁是 ADR-0113 §4 第二条的直接后果：浏览器只有一个上下文，没有「哪个会话的模型在看它」
可分。

**做完的判据**：真实转录里出现「想在面板里连按却卡顿」的反馈时再评估推流。

**2026-09-13 补记（ADR-0119）**：这一条的后半截——「任何会话有回合在跑就点不了」——没有了。用户的
原话是「还存在浏览器状态一直被控制，人无法操作的状态」：一个「写完页面再去浏览器里验」的回合要跑
几分钟，而人想操作的正是那几分钟。锁换成了一句话：人随时能动，模型在它下一次浏览器调用上被告知
页面被人碰过、ref 要重新取。剩下的代价是轮询的节奏（250 ms 一帧）和「两边可能撞在同一瞬间」，
后者的粒度是一次调用的毫秒级，不是一轮的分钟级。

**2026-09-13 再补记（ADR-0120）**：「按住方向键连发靠的是键盘重复事件逐个送」这一句描述的正是一个
**坏掉**的控制，而不只是一个慢的：每个键送成一次 `keyboard.press`，按下和抬起一口气做完，读键状态
的页面（马里奥的 `keys.right`）一帧都看不见它按着——用户的原话是「人还是无法控制在浏览器中的项目
预览」。现在按下、抬起分开送（`key_down` / `key_up`，鼠标同理），排队按序、不再丢，失去焦点时把还
按着的松开；焦点下取帧从 250 ms 改成 100 ms，上一张回来才排下一张。剩下的代价只是轮询本身：10 帧/
秒能走、能跳、能看清停在哪，打不了要求反应的游戏。

### F-44 Code 会话跨回合只带用户消息与最终报告，工具结果不带 —— 缺口

**证据**：[code_session.py](../src/agent_workbench/application/code_session.py) 在一轮之前
`append` 用户消息，之后只在 `outcome.output_text` 非空时 `append` 一条助手消息；下一轮的
`history` 就是这些。2026-09-13 的三轮「请你编写马里奥」（ADR-0118 §1）：第二轮开头模型说
"my last turn's reads may not be in context anymore"，从头重读全部文件，`RunCompleted.usage`
是 338,782 输入 token；第一轮 `RunFailed` 没有报告，第三轮不知道第一轮是怎么失败的，
用同一种方式又失败了一次。

**Claude Code 怎么做**：整份 transcript——用户消息、助手文本、每一次工具调用与结果——留在
上下文里直到自动压缩；一轮失败了，下一轮看得见失败的那一步。

**为什么还没做**：把工具结果带过回合要动会话存储（现在存的是 `Message`，工具结果是回合内
`_RunLedger.messages` 的事）、DeepSeek 适配器的消息序列化（工具调用与结果要成对回放）、以及
ADR-0116 的按记录作答（记录目前只活在一轮之内）。一条自己的 ADR。ADR-0118 先把截断这一种失败
收在**同一轮**里处理，让最常见的那种「下一轮不知道」不再发生。

**做完的判据**：一轮的工具结果出现在下一轮的第一次模型请求里，且一轮失败之后下一轮的第一句
不再是「先重读一遍」。

### F-45 步数用尽的一轮不留报告 —— **已关闭**（2026-09-13，ADR-0119）

**证据**：第八十六批 §2.1 的第二轮（`ses_a42b…`）：60 步、61 次调用，`mario.html` 八段写完、
`python3` 核对过、浏览器里修了三个 bug、刚验证完方块碰撞，在测金币收集时
`RunFailed: the run stopped at its ceiling: max_steps`。`code_session.py` 只在 `outcome.output_text`
非空时把助手消息写进会话，失败的一轮没有 `output_text`，于是控制台上是「这一轮没有跑完
（max_steps）」，会话历史里什么也没有，下一轮的模型不知道文件已经在那里。

**Claude Code 怎么做**：没有步数上限这个概念（有 `--max-turns`，到了就停），但整份 transcript
留在上下文里，下一轮接着上一轮最后一步。

**修法（ADR-0119）**：`RunBudget.report_turn_due` —— 最后一步预留给报告而不是用来干活，
`max_steps >= 2 且 steps >= max_steps - 1` 时这一轮不带工具、消息末尾追加一条说明。是**预留**不是
多给：一轮仍然正好花掉 `max_steps`。仍然以 `stop_reason=max_steps` 失败（活没干完，而这套运行时
也跑 Task 图的节点），但报告随 `output_text` 出来，会话把它追加进历史。只有步数能这么留：时间、
token、钱跑完的一轮连那一次调用都付不起。Code 的三个默认值同批放大（步数 60→120、调用 120→240、
一轮 600→900 秒）。

**判据已满足**：`test_the_step_ceiling_buys_a_turn_to_write_the_report`（报告出得来、正好 `max_steps`
步、收尾请求不带工具）与 `test_the_closing_turn_is_taken_once_and_only_for_steps`（截止时间不买）。

### F-46 循环守卫只认「只改数字」这一种形状 —— 已知代价

**证据**：[agent_runtime.py](../src/agent_workbench/runtime/agent_runtime.py) 的 `_call_shape` 把参数里的数字
和字符串里的数字串抹成 `#`，同一形状第 8 次起拒绝（ADR-0121）。一次非只读派发会清掉**这一批没派发过的**
形状的计数，这是「跑测试、改代码、跑测试」不被当成循环的原因——也是交替循环（形状 A 的命令、形状 B 的命令、
A、B……，两者都是非只读）永远攒不够数的原因。只改字母、不改数字的一格一格量（`doc_a`、`doc_b`……）也不算
同一形状。

**为什么算已知代价**：2026-09-13 实际发生的两种循环（一条 `python3 -c` 只改偏移量 113 次；ADR-0114 那一轮用
正则量词二分一个长度）都是单一形状、只改数字。交替循环和变字母的循环还没在记录里出现过；为它们设计的规则
（比如按「最近 N 次调用里形状种类很少」判断）会碰到大量正常的工作节奏，要先有记录才能校准。ADR-0114 的
每 25 次一句提醒对它们仍然有效。

**做完的判据**：事件记录里出现一个交替形状或变字母的循环（或者开了 `record_step_inputs` 的部署能看到参数本身）
时，用那份记录校准第二种规则。
