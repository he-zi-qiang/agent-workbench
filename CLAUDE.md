...
- `apps/` — `agent-api`, `agent-cli`, `agent-task-worker`, `agent-ingestion-worker`, `agent-encoder` (the one model-holding process, ADR-0106), plus **four** project-owned MCP servers (`word`, `web`, `sandbox`, `computer`). Entry points are in `pyproject.toml` `[project.scripts]`.
