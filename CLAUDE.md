...
`scripts\stack.cmd` (Windows) and `docker build … && docker compose --profile demo up -d --wait` are the same stack. **Since ADR-0105 it is not the deliberately light one**: the image carries `--extra embedding`, `AW_CONFIG_FILE` names `config.compose-local.toml`, each Task Worker starts the Word and web MCP servers as loopback sidecars and probes them with `scripts/smoke_mcp_server.py` before it execs (the catalogue freezes once, and discovery failure is fail-soft), the ingestion worker dropped `--demo`, and a one-shot `weights-init` fills a named HF cache — **the sparse arm refuses to start on a cold cache rather than filling it**.

**One process loads the models: the `encoder` service (ADR-0106).** The API, both Task Workers and the ingestion worker set `rag.embedding.service_url` / `rag.reranker.service_url` to `http://encoder:8769` and get `RemoteEmbedder` / `RemoteSparseEncoder` / `RemoteReranker` from the same three factories; they mount no HF cache and import no torch. The one measured number is `~12 GB` for *one* model-holding process (2026-07-31, `docs/running-locally.md`); `stack.cmd` compares `docker info`'s `MemTotal` against 12 GB (that figure) and 16 GB (that figure plus a stated allowance — decimal, because `set /a` is 32-bit signed and overflows on a byte count) and stops below the floor rather than after a thirty-minute build. The native path (`scripts/dev.sh`) still loads in-process; the two leaves are what would move it.

**The sandbox is in the topology since ADR-0107**, by a shape rather than a mount: the `sandbox` service alone mounts `/var/run/docker.sock` (a test holds it to exactly one), runs `agent-sandbox-mcp` and nothing else, and is reached through `docker/loopback_proxy.py` tunnels whose both ends are loopback — so the servers' `--host` choice lists, the MCP SDK's Host check and the settings validator all hold unchanged. `code.sandbox_enabled` / `[sandbox].enabled` stay `false` in the profile; `docker/decide_sandbox.py` lets each launcher turn them on per start once the broker's *runtime* answers (the same shape as `decide_web_search.py`). The image carries the Docker CLI for this.

**Computer use is the one part outside the containers (ADR-0108).** `adapters/screen/win32.py` is a second `ScreenPort` (ctypes only; the Windows half of the `computer-use` extra is Pillow), the server runs on the Windows host via `scripts\computer.cmd`, and the API reads its `/session` through a tunnel to `host.docker.internal:8768`. The frame is *composed* from per-window `PrintWindow` renders of approved windows (that is how `exclude_native` is kept there); the platform half has never run on Windows. `docs/windows-quickstart.md` is the Chinese route in from a bare machine.
...
`config/config.<name>.toml`, selected by `AW_CONFIG_FILE`. **Eleven of them**: `local` (no MCP), `word-local`, `web-local`, `code-local`, `computer-local`, `sandbox-local`, `demo-local` (the union, what the native console runs), `compose-local` (what the Compose stack runs — ADR-0105; it leaves `code.sandbox_enabled`, `code.browser_enabled` and `runner.enabled` false in the file because every one of those slots is fail-fast, and `docker/run-api-local.sh` probes the sandbox broker, the browser and the runner containers per start and exports the switch for each that answers — ADR-0107, ADR-0115), plus `default`/`test`/`production`. The profiles are deliberately separate files: each freezes its own tool names into every Task authorization envelope at submission, so a wider profile widens every Task.
...
Single schema (currently `1.20`), cross-domain validation at startup: a capability the config claims but the code does not have **fails at config load**. `docs/configuration.md` §3 lists the invariants written as single-valued `Literal`s in `bootstrap/settings.py` — PostgreSQL as the fact source, `FOR UPDATE SKIP LOCKED`, fusion owned by the application, the self-built runtime as the only tool loop, telemetry body recording off, and more. **Changing one of those requires an ADR first**, not an environment override.

### Where things live

- `apps/` — `agent-api`, `agent-cli`, `agent-task-worker`, `agent-ingestion-worker`, `agent-encoder` (the one model-holding process, ADR-0106), plus **six** project-owned MCP servers (`word`, `web`, `sandbox`, `computer`, `browser` since ADR-0113, `runner` since ADR-0115 — the last is the shell a Compose coding session gets: one `run_command` in a container that mounts only the project folder). Entry points are in `pyproject.toml` `[project.scripts]`.
- `adapters/` — one directory per outer concern, **23 of them**: `artifacts`, `concurrency`, `documents`, `embedding`, `encoder` (the HTTP clients of `agent-encoder`), `evaluation`, `filesystem`, `ingestion`, `langgraph`, `llama_index`, `mcp`, `memory` (the in-memory doubles), `models`, `persistence`, `policy`, `reranking`, `research`, `retrieval`, `screen` (`darwin.py` and `win32.py`), `telemetry`, `testing`, `tools`, `vector` — plus two loose modules, `delegation.py` and `events.py`. The list here used to name eleven, and the omissions were not the small ones: `research/` is 1310 lines, `documents/` 835, `filesystem/` 791, `screen/` 773.
...
- `web/src/features/` — `chat`, `code`, `work`, `knowledge`, `evaluation`, `computer`, `system`, `usage`. The shell (`web/src/app/`) also owns the rail's 待处理 entry (`PendingApprovals.tsx`, reads `GET /v1/approvals?status=pending`) and the three-tier page layering rule from the 2026-09-04 review (`docs/reviews/`): state and next action first, evidence second, ADRs and protocol fields folded under `EngineeringNotes`. **`computer` does read an endpoint** (ADR-095): `routes/computer.py` serves `GET /session` through a read-only reverse proxy, `main.py` mounts it unconditionally, and the page polls it every 4 seconds for the allowlist, the frontmost application and recent actions. This line said the opposite until 2026-08-31, which mattered because it was the premise a reader would use to decide whether to *add* an endpoint for that page.
